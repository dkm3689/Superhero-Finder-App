...
//                 local storage
//                 localFavouritesSet.delete(characterToAdd);

//                 localStorage.setItem(
//                   "localFavouritesSet", JSON.stringify(Array.from(localFavouritesSet))
//                   );
//                 });
...
//   // const localFavouritesSet = JSON.parse(localStorage.getItem("localFavouritesSet"));

//   let localFavouritesSet = new Set(
//     JSON.parse(localStorage.getItem("localFavouritesSet")) || []
//   );

//     localFavouritesSet.forEach((character) => {
//       const favouritesCard = document.createElement("div");
//       favouritesCard.innerHTML = `<h2> ${character.name} </h2>`;
//       favouritesCard.classList.add("favourites-card");
//       favouritesRow.appendChild(favouritesCard);

//       const favouritesImage = document.createElement("img");
//       favouritesImage.src = `${character.image}`;
//       favouritesImage.classList.add("hero-image");
//       favouritesCard.appendChild(favouritesImage);

//       const favouritesRemoveButton = document.createElement("button");
//       favouritesRemoveButton.innerHTML = "Remove";
//       favouritesRemoveButton.classList.add("remove-button");
//       favouritesCard.appendChild(favouritesRemoveButton);

//       favouritesRemoveButton.addEventListener("click", () => {
//         favouritesRemoveButton.innerHTML = "Removed";
//         favouritesSet.delete(character.name);

//         if (favouritesSet.size === 0) {
//           favouritesEmptyMessage.style.display = "block";
//         }

//         favouritesCard.remove();

//         localFavouritesSet.delete(character);
...
let favouritesSet = new Set(
  JSON.parse(localStorage.getItem("favouritesSet")) || []
);

// Example: Logging a value to the console
const myValue = "Hello, Local Storage!";
console.log("Value to be stored:", myValue);

// Store the value in local storage
localStorage.setItem("myKey", myValue);

// Retrieve the value from local storage and log it
...
favouritesButton.addEventListener("click", () => {
  ...
  if (favouritesSet.size === 0 && localFavouritesSet.size === 0) {
    favouritesEmptyMessage.style.display = "block";
  } else {
    favouritesEmptyMessage.style.display = "none";
  }
    ...
    .then((data) => {
      ...
      if (data.data.results.length > 0) {
        ...
        data.data.results.forEach((character) => {
          ...
          // if (
          //   favouritesSet.has(character.name) ||
          //   localFavouritesSet.has(character.name)
          // ) {
          //   addToFavourites.textContent = "Added to Favourites";
          //   addToFavourites.classList.add("added-to-favourites");
          // }
          ...
          addToFavourites.addEventListener("click", (event) => {
            ...
            if (
              ...
              addToFavourites.classList.add("added-to-favourites"); // Add a class to the button
              addToFavourites.textContent = "Added To Favourites";
              favouritesSet.add(character.name); // Add the character to the set
...
retrieveFavourites(localFavouritesSet);

function retrieveFavourites(localFavouritesSet) {
  // const localFavouritesSet = JSON.parse(localStorage.getItem("localFavouritesSet"));
  ...
  localFavouritesSet.forEach((character) => {
    ...
    heroName.classList.add("hero-name");
    favouritesCard.appendChild(heroName);
